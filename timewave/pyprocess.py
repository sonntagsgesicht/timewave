...
class Step_process(object):
    ...
    def _check_time(self, t):
        if t < self.startTime:
            raise Exception(
                "Attn: inputed time not valid. Check if beginning time is less than startTime (0 by default).")
...
class Poisson_process(Renewal_process):
    ...
    def _sample_position(self, t, N=1):
        if self.conditional:
            return self.Bin.rvs(self.endPosition - self.startPosition, float(t) / self.endTime,
                                size=N) + self.startPosition
        else:
...
class Diffusion_process(object):
    ...
    def transition_pdf(self, t, y):
        ...
        try:
            if not self.conditional:
                ...
                return self._transition_pdf(self.startPosition, t - self.startTime, y) * self._transition_pdf(y,
                                                                                                              self.endTime - t,
                                                                                                              self.endPosition) \
    ...
    def _var(self, t):
        ...
        """
        return self.sample_position(t, int(1e6)).var()

    def _mean(self, t):
        return self.expected_value(t)

    def _sample_position(self, t, N):
        return self.Euler_scheme(t, N)
    ...
    def Euler_scheme(self, times, delta=0.001):
        ...
        for i in xrange(steps):
            if t + delta > time > t:
                delta = time - t
                x += self.drift(x, t) * delta + np.sqrt(delta) * self.diffusion(x, t) * Nor.rvs()
                path.append((x, time))
                delta = 0.001
                j += 1
                time = times[j]
...
class Wiener_process(Diffusion_process):
    ...
    def _mean(self, t):
        if self.conditional:
            ...
            return self.startPosition + self.mu * delta1 + (
                                                           self.endPosition - self.startPosition - self.mu * delta2) * delta1 / delta2
        else:
    ...
    def _sample_path(self, times, N=1):
        ...
        if not self.conditional:
            ...
            for i, delta in enumerate(deltas):
                x = x * (1 - delta / T) + (self.endPosition - self.startPosition) * delta / T + self.sigma * np.sqrt(
                    delta / T * (T - delta)) * self.Nor.rvs(N)
                T = T - delta
                path[:, i] = x
            if abs(T - 0) < 1e-10:
...
class OU_process(Diffusion_process):
    ...
    def _mean(self, t):
        if self.conditional:
            ...
            return self.startPosition * np.exp(-self.theta * (t - self.startTime)) + self.mu * (
            1 - np.exp(-self.theta * (t - self.startTime)))
    ...
    def _sample_position(self, t, n):
        if not self.conditional:
            return self.mean(t) + np.sqrt(self.var(t)) * self.Normal.rvs(n)
        else:
            # this needs to be completed
            return super(OU_process, self).sample_position(t)
...
class Integrated_OU_process(Diffusion_process):
    ...
    def __init__(self, parameters, time_space_constraints):
        ...
        self.OU = OU_process({"theta": parameters["theta"], "mu": parameters["mu"], "sigma": parameters["sigma"]},
                             {"startTime": time_space_constraints["startTime"], "startPosition": parameters["x0"]})
        for p in parameters:
    ...
    def _transition_pdf(self, x, t, y):
        mu = x + (self.x0 - self.mu) / self.theta + self.mu * t - (self.x0 - self.mu) * np.exp(
            -self.theta * t) / self.theta
        sigmaSq = self.sigma ** 2 * (
        2 * self.theta * t - 3 + 4 * np.exp(-self.theta * t) - 2 * np.exp(-2 * self.theta * t)) / (2 * self.sigma ** 3)
        return np.exp(-(y - mu) ** 2 / (2 * sigmaSq)) / np.sqrt(2 * pi * sigmaSq)
        ...
        if not self.conditional:
            ...
            for i, position in enumerate(xPath):
                ...
                if delta != 0:
                    ...
                    muY = y + (x - self.mu) / self.theta + self.mu * delta - (x - self.mu) * np.exp(
                        -self.theta * delta) / self.theta
                    covXY = self.sigma ** 2 * (
                    1 + np.exp(-2 * self.theta * delta) - 2 * np.exp(-self.theta * delta)) / (2 * self.theta ** 2)
                    y = muY + np.sqrt(sigmaY - covXY ** 2 / sigmaX) * self.Normal.rvs() + covXY / np.sqrt(sigmaX) * \
                                                                                          listOfNormals[i]
                t = position[0]
                path.append((t, y))
            if returnUO == 0:
...
class SqBessel_process(Diffusion_process):
    ...
    def _transition_pdf(self, x, t, y):
        try:
            return (y / x) ** (0.5 * self.mu) * np.exp(-0.5 * (x + y) / self.nu ** 2 / t) / (
            0.5 * self.nu ** 2 * t) * iv(abs(self.mu), 4 * np.sqrt(x * y) / (self.nu ** 2 * t))
        except AttributeError:
...
class CIR_process(Diffusion_process):
    ...
    def __init__(self, parameters, space_time_constraints):
        ...
        _space_time_constraints['startPosition'] = self._inverse_space_transformation(
            space_time_constraints['startTime'], space_time_constraints['startPosition'])
        try:
            _space_time_constraints['endPosition'] = self._inverse_space_transformation(
                space_time_constraints['endTime'], space_time_constraints['endPosition'])
            _space_time_constraints['endTime'] = self._time_transformation(space_time_constraints['endTime'])
        except:
            pass
        self.SqB = SqBessel_process({"lambda0": parameters["lambda_0"], "nu": parameters["nu"]},
                                    _space_time_constraints)  # need to change start position for non-zero startTime
        self.mu = self.SqB.mu
    ...
    def _transition_pdf(self, x, t, y):
        return np.exp(self.lambda_1 * t) * SqB._transition_pdf(x, self._time_transformation(t),
                                                               np.exp(self.lambda_1 * t) * y)
...
class Periodic_drift_process(Diffusion_process):
    ...
    def __init__(self, parameters, space_time_constraints):
        ...
        self.Poi = Marked_poisson_process(
            {"rate": 1, "U": self.max, "L": self.min, "startTime": 0})  # need to create marked poisson process class
        self.Nor = stats.norm
    ...
    def phi(self, x):
        return (0.5 * self.psi * self.psi * sin(x - self.theta) ** 2 + 0.5 * self.psi * cos(
            x - self.theta) - self.min) / self.max
...
class GBM_process(Diffusion_process):
    ...
    def __init__(self, mu, sigma, startPosition=1, startTime=0, endPosition=None, endTime=None):
        ...
        try:
            wienerConstraints = {"startTime": self.startTime, "endTime": self.endTime, "startPosition": 0,
                                 "endPosition": np.log(self.endPosition / self.startPosition)}
        except:
    ...
    def _mean(self, t):
        if not self.conditional:
            ...
            return self.startPosition * np.exp(
                -0.5 * (self.sigma * t) ** 2 / delta + self.Weiner.endPosition * t / delta)

    def _transition_pdf(self, x, t, y):
        delta = t - self.startTimee
        d = (self.mu - 0.5 * self.sigma ** 2)
        return x / (y * np.sqrt(2 * Pi * self.sigma ** 2 ** delta)) * np.exp(
            -(np.log(y / x) - d) ** 2 / (2 * self.sigma ** 2 * delta))
    ...
    def _var(self, t):
        if not self.conditional:
            ...
            return self.startPosition ** 2 * np.exp(
                2 * self.mu * t - self.sigma ** 2 * t + self.sigma * t / delta * (self.sigma * (delta - T) + 2 * X)) * (
                   np.exp(self.sigma ** 2 * (delta - t) * t / delta) - 1)
...
class Gamma_process(Jump_Diffusion_process):
    ...
    def _generate_position_at(self, t):
        if not self.conditional:
            return self.startPosition + self.gamma.rvs(
                self.mean ** 2 * (t - self.startTime) / self.variance) * self.mean / self.variance
        else:
            return self.startPosition + (self.endPosition - self.startPosition) * self.beta.rvs(
                (t - self.startTime) / self.variance, (self.endTime - t) / self.variance)

    def _get_mean_at(self, t):
        "notice the conditional is independent of self.mean."
        if self.conditional:
            return self.startPosition + (self.endPosition - self.startPosition) * (t - self.startTime) / (
    ...
    def _transition_pdf(self, x, t, y):
        return self.variance / self.mean * self.gamma.pdf((y - x) * self.mean / self.variance * t,
...
class Geometric_gamma_process(Jump_Diffusion_process):
    ...
    def __init__(self, parameters, space_time_constraints):
        ...
        try:
            self.gammaProcess = Gamma_process({"mean": self.mu, "variance": self.sigma},
                                              {"startPosition": 0, "startTime": 0, "endTime": self.endTime,
                                               "endPosition": np.log(self.endPosition / self.startPosition)})
        except:
            self.gammaProcess = Gamma_process({"mean": self.mu, "variance": self.sigma},
                                              {"startPosition": 0, "startTime": 0})
    ...
    def _transition_pdf(self, z, t, x):
        ...
        return (np.log((z - x) / self.startPosition)) ** (alpha - 1) * ((z - x) / self.startPosition) ** (-1 / beta) / (
        (z - x) * beta ** alpha * gamma(alpha))
    ...
    def _get_mean_at(self, t):
        if self.conditional:
            ...
            if self.sigma < self.mu:  # this condition gauruntees the expectation exists
                return self.startPosition * (1 - self.sigma / self.mu) ** (
                -self.mu ** 2 * (t - self.startTime) / self.sigma)
            else:
...
class Inverse_Gaussian_process(Jump_Diffusion_process):
    ...
    def _transition_pdf(self, z, t, x):
        y = x - z
        return self.a * (t - self.startTime) / (2 * pi) * (y) ** (-1.5) * np.np.exp(
            -0.5 * ((self.a * (t - self.startTime)) ** 2 / y + self.b ** 2 * y) + self.a * (
            t - self.startTime) * self.b)
...
class Normal_Inverse_Gaussian_process(Jump_Diffusion_process):
    ...
    def _get_mean_at(self, t):
        return self.startPosition + self.delta * (t - self.startTime) * self.beta / (
    ...
    def _transition_pdf(self, z, t, x):
        ...
        return self.alpha * self.delta * (t - self.startTime) * kn(1, self.alpha * y) * np.np.exp(
            self.delta * gamma + self.beta * (z - x)) / (pi * y)
